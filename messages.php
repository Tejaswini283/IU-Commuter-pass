...
<!--STRIPE-->
<div class="col-md-12">
<h4 class="section-subtitle"><b><?php if(isset($_GET['add'])){ echo'Start Ride';  } else{  echo'Update Previous Ride'; } ?></b> Course</h4>
<div class="panel">
...
$sessionid = htmlspecialchars($_SESSION['user_id']);
  ...
  $p_id = $_GET['p_id'];
  $select = mysqli_query($con,"SELECT contact.*,p.Id as pid,s.first_name as s_fname,s.last_name  as s_lname FROM contact left JOIN users as p on p.id=contact.passenger_id left JOIN users as s on s.id=contact.sender_id left JOIN users as r on r.id=contact.rider_id WHERE contact.date=curdate() and contact.rider_id='$sessionid' and contact.passenger_id='$p_id' order by contact.id desc");
...
if(isset($_POST['msg'])){
   ...
   $run= mysqli_query($con,"INSERT INTO `contact`(`passenger_id`, `rider_id`, `msg`, `date`, `time`, `sender_id`) VALUES ('$p_id','$rider','$msg',now(),now(),'$rider')");

   if($run){
    echo"<script>window.open('messages.php?p_id=$p_id','_self')</script>";
}
}?>
